...
const NFT = await stdlib.launchToken(accountCreator, 'real estate', 'RES', { supply: 1 });
...
const startBidders = async () => {
    ...
    const runBidder = async (who) => {
        ...
        try {
            const [ lastBidder, lastBid, bidStatus ] = await ctc.apis.Bidder.makeBid(bid);
            ...
            if (bidStatus == 0) {
                ...
                if (intervals[who] === 0) {
                    intervals[who] = setInterval( async () => {
                        try {
                            const [foreRunner, amount, status] = await ctc.apis.Bidder.seeBids();
        
                            if (status == 1) {
                                console.log(`${who} sees that ${foreRunner} is currently winning with a bid of ${fmt(amount)}`);
                            }
                        }
                        catch(err) {
                            console.log(`${who} tried seeing the bids but could not because the bid has ended.`);
                ...
                console.log(`${who} made a bid that is not greater than the current minimum bid of ${fmt(lastBid)}`);
            }
